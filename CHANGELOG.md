...
- **Goal 目标指引（`main.py`）**：新增会话级 Goal 管理功能，支持用户与 LLM 双侧设置/清除当前对话的目标指引，在每次 LLM 请求时自动注入，引导对话方向。
  - **用户命令**：
...
- **最小触发间隔**：新增 `content_audit_min_interval` 配置，关键词触发时若距上次审核不足最小轮数则设保持位等待，达标后再触发。
...
- **`content_audit_min_interval` 配置项**（int, 默认 2）— 最小触发间隔轮数。防止刚审核完又被关键词触发。
